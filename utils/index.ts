export const watchTimePassedOut = (createdAt: any) => {
  ...
  const feedCreatedAt = new Date(createdAt);
  const now = new Date();
  const diff = now.getTime() - feedCreatedAt.getTime();
  ...
  const formattedDate = new Date(feedCreatedAt).toLocaleDateString(userLocale, {
    month: "long",
    day: "numeric",
    year: "numeric",
  });
